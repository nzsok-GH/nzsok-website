...
export default function HeroPhysics() {
  ...
  useEffect(() => {
    ...
    // Builds the whole simulation for the current section size and returns a
    // teardown fn. Re-run on width change so the canvas never keeps stale dims.
    const setup = () => {
    ...
    // Re-init only when the width actually changes (orientation / window resize).
    // Ignore height-only changes so the mobile URL bar doesn't thrash the canvas.
    let resizeRaf = 0;
    const onResize = () => {
