...
export default function Gallery() {
  ...
  const prevPhoto = useCallback(() => setLightbox(lb => lb && lb.photoIdx > 0 ? { ...lb, photoIdx: lb.photoIdx - 1 } : lb), [])
  const nextPhoto = useCallback(() => setLightbox(lb => lb && lb.photoIdx < lb.album.photos.length - 1 ? { ...lb, photoIdx: lb.photoIdx + 1 } : lb), [])
        ...
        <section id="album">
          ...
          {loading ? (
            ...
            <div className="text-center py-24">
              ...
              <h3 style={{ fontFamily: "'SUIT', sans-serif", fontSize: 22, color: '#1c2b3a' }}>아직 앨범이 없어요</h3>
            </div>
          ) : (
            years.map(year => (
              <div key={year} style={{ marginBottom: 64 }}>
                <div className="flex items-center gap-4 mb-7">
                  <span style={{ fontFamily: "'SUIT', sans-serif", fontSize: 28, fontWeight: 900, color: '#1c2b3a' }}>{year}</span>
                  <div style={{ flex: 1, height: 1, background: 'rgba(0,0,0,0.08)' }} />
                  <span style={{ fontSize: 12, color: '#8a9ab0', fontWeight: 500 }}>앨범 {byYear[year].length}개</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
                  {byYear[year].map(album => (
                    ...
                    >
                      <div className="album-thumb relative" style={{ aspectRatio: '4/3', overflow: 'hidden' }}>
                        {album.cover ? (
                        ...
                        {album.photos.length > 0 && (
                          <span
                            ...
                            style={{ background: 'rgba(146,120,214,0.85)', backdropFilter: 'blur(8px)' }}
                          >
                      ...
                      <div style={{ padding: '18px 20px' }}>
                        <div style={{ fontSize: 12, color: '#8a9ab0', marginBottom: 5 }}>{formatDate(album.date)}</div>
                        <div style={{ fontFamily: "'SUIT', sans-serif", fontSize: 16, fontWeight: 700, color: '#1c2b3a', marginBottom: 5, lineHeight: 1.4 }}>
                          {album.title}
                        </div>
                        {album.description && (
          ...
          <div style={{ textAlign: 'center', paddingTop: 48 }}>
            <a
              href="/admin/"
              style={{ color: '#c4cdd6', fontSize: 12, textDecoration: 'none', transition: 'color 0.2s' }}
        ...
        <section id="instagram" style={{ marginTop: 96 }}>
          <div className="flex items-center justify-between flex-wrap gap-4 mb-14">
            ...
            <a
              ...
              style={{ fontSize: 14, fontWeight: 600, color: '#9278D6', transition: 'opacity 0.2s' }}
      ...
      {lightbox && (
        ...
        >
          <div
            ...
            style={{ maxWidth: 1000, maxHeight: '90vh', display: 'grid', gridTemplateColumns: '1fr 340px' }}
          >
            <div className="relative flex items-center justify-center" style={{ background: '#000', minHeight: 400 }}>
              ...
              {lightbox.photoIdx > 0 && (
                <button
                  ...
                  style={{ left: 16, top: '50%', transform: 'translateY(-50%)', width: 44, height: 44, background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', fontSize: 18 }}
                  onClick={prevPhoto} aria-label="이전 사진"
                >‹</button>
              )}
              {lightbox.photoIdx < lightbox.album.photos.length - 1 && (
                <button
                  className="absolute flex items-center justify-center rounded-full cursor-pointer"
                  style={{ right: 16, top: '50%', transform: 'translateY(-50%)', width: 44, height: 44, background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', fontSize: 18 }}
                  onClick={nextPhoto} aria-label="다음 사진"
                >›</button>
              )}
              <div
            ...
            <div className="flex flex-col overflow-y-auto" style={{ padding: '36px 32px' }}>
              <button
                ...
                onClick={closeLightbox} aria-label="닫기"
              >✕</button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
                {lightbox.album.photos.map((photo, i) => (
                  <div
                    ...
                    style={{ aspectRatio: '1', border: `2px solid ${i === lightbox.photoIdx ? '#9278D6' : 'transparent'}`, transition: 'all 0.15s' }}
